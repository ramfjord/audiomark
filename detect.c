...
{ //{{{
  ...
  for(int i = 0; i < noise_len; i++){
		int i_i = embed_indices[i];
    ex_elt = extract_wmark_elt(orig_freq_buffer[i_i], test_freq_buffer[i_i]);
	...
	if(test_iteration == test_debug) printf("\n");
...
{ //{{{
  ...
  orig_time_buffer = (double *) malloc(sizeof(double)*BUFFER_LEN*channels);
  orig_freq_buffer = (complex *) fftw_malloc(sizeof(complex)*FREQ_LEN*channels);

  test_time_buffer = (double *) malloc(sizeof(double)*BUFFER_LEN*channels);
  test_freq_buffer = (complex *) fftw_malloc(sizeof(complex)*FREQ_LEN*channels);
  ...
  // set up transform plans
	...
	ft_test = (fftw_plan *)malloc(sizeof(fftw_plan) * channels);

	for(int i = 0; i < channels; i++){
		ft_orig[i] = fftw_plan_dft_r2c_1d(BUFFER_LEN,orig_time_buffer+BUFFER_LEN*i, 
																	orig_freq_buffer+FREQ_LEN*i, FFTW_ESTIMATE);
		ft_test[i] = fftw_plan_dft_r2c_1d(BUFFER_LEN,test_time_buffer+BUFFER_LEN*i, 
																	test_freq_buffer+FREQ_LEN*i, FFTW_ESTIMATE);
	}
  ...
  while(1){
    ...
    orig_bytes_read = sf_read_double(s_orig, orig_time_buffer, BUFFER_LEN*channels);
    test_bytes_read = sf_read_double(s_test, test_time_buffer, BUFFER_LEN*channels);

    // no information will be encoded in the last "frame" - if we don't read in BUFFER_LEN
    // bytes, then there will be no embedded information there
    if(test_bytes_read != BUFFER_LEN*channels || 
			 orig_bytes_read != BUFFER_LEN*channels)
      break;

		deinterleave_channels(orig_time_buffer, channels);
		deinterleave_channels(test_time_buffer, channels);

		for(int i = 0; i < channels; i++){
			fftw_execute(ft_orig[i]);
			fftw_execute(ft_test[i]);
		}

    if(counter == test_debug){
    ...
    if(wmark->type == FH_EMBED)
      ...
      ss_extract_watermark(orig_freq_buffer, test_freq_buffer, 
													 FREQ_LEN*channels, &o_dot_x, &x_dot_x);
  ...
  //
	for(int i = 0; i < channels; i++){
		fftw_destroy_plan(ft_test[i]);
		fftw_destroy_plan(ft_orig[i]);
	}
	free(ft_test);
	free(ft_orig);
